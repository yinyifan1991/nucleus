...
//high order cliques
typedef tuple<vertex, vertex, vertex, vertex> v4c;
typedef tuple<vertex, vertex, vertex, vertex, vertex> v5c;
typedef tuple<vertex, vertex, vertex, vertex, vertex, vertex> v6c;
typedef tuple<vertex, vertex, vertex, vertex, vertex, vertex, vertex> v7c;
typedef tuple<vertex, vertex, vertex, vertex, vertex, vertex, vertex, vertex> v8c;
typedef tuple<vertex, vertex, vertex, vertex, vertex, vertex, vertex, vertex, vertex> v9c;

/*
 * a tree structure to store cliques
...
public:
    ...
    int order;
    int id;
    vector<com_node> nodes;
    com_node(){};
    com_node(vertex val, int order, int id) {
        this->val = val;
        this->order = order;
        this->id = id;
    }
...
inline void createOrderedIndexEdgesNode (Graph& graph, com_node *root, Graph& orderedGraph) {
    orderedGraph.resize(graph.size());
    int count = 0;
    for (size_t u = 0; u < graph.size(); u++) {
        com_node cur(u, 1, u);
        root->nodes.push_back(cur);
        for (size_t j = 0; j < graph[u].size(); j++) {
            vertex v = graph[u][j];
            if (less_than (u, v, graph)) {
                orderedGraph[u].push_back(v);
                //cur.nodes.push_back(com_node(v, 2, count));
                root->nodes[u].nodes.push_back(com_node(v, 2, count));
				//cout << u << " " << v << endl;
                count++;
            }
        }
...
void base_k67_gen (Graph& graph, bool hierarchy, edge nEdge, vector<vertex>& K, vertex* max45, string vfile, FILE* fp);
void base_k78_gen (Graph& graph, bool hierarchy, edge nEdge, vector<vertex>& K, vertex* max45, string vfile, FILE* fp);
