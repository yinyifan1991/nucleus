...
void traverse_cliqueTree (com_node* parent, vector<com_node>& ancestors, Graph& orderedGraph, Graph& orderedCs, vector<v8c>& rcs, vector<vertex>& xrcs, Graph& scs, vector<int>& id_in_level, int rounds) {
    if(rounds < 0) return;
    for(int i = 0;i < parent->nodes.size();i++) {
        ...
        if(rounds > 0) {
            ...
            for (auto x : commonNeighbors) {
                ...
                v8c cl = make_tuple (ancestors[0].val, ancestors[1].val, ancestors[2].val, ancestors[3].val, ancestors[4].val, ancestors[5].val, ancestors[6].val, (vertex)x);
                rcs.push_back (cl);
            }
...
void create_rcliqueList (com_node *root, Graph& orderedGraph, Graph& orderedCs, vector<v8c>& rcs, vector<vertex>& xrcs, Graph& scs, vector<int>& id_in_level, int rds) {
    vector<com_node> ancestors;
    for(size_t i1 = 0;i1 < root->nodes.size();i1++) {
        com_node *u1 = &root->nodes[i1];
        ancestors.push_back(*u1);
        traverse_cliqueTree(u1, ancestors, orderedGraph, orderedCs, rcs, xrcs, scs, id_in_level, rds-2);
        ancestors.pop_back();
    }
}


void create_cliqueList(com_node *root, com_node* parent, vector<com_node>& ancestors, Graph& orderedGraph, Graph& orderedCs, vector<v8c>& rcs, vector<vertex>& xrcs, Graph& scs, vector<int>& id_in_level, int rounds) {
    if(rounds > ROUNDS) {
        return;
    }
    vector<vector<vertex>> vList;
...
// per 8-clique
lol count9cliques_gen (com_node *root, Graph& graph, Graph& orderedGraph, Graph& orderedCs, vector<v8c>& rcs, Graph& scs) {
    lol fc = 0;
    for (auto t : rcs) {
        ...
        while (i1 < orderedGraph[u1].size() && i2 < orderedGraph[u2].size() && i3 < orderedGraph[u3].size()
            ...
            if (a1 == a2 && a1 == a3 && a1 == a4 && a1 == a5 && a1 == a6 && a1 == a7 && a1 == a8) {
                ...
                increment_clique(root, {u1, u2, u3, u4, u5, u6, u7, u8, x}, orderedCs, scs, clique, 0, 0, 1);
...
lol create_cliques_and_count(com_node *root, Graph& graph, Graph& orderedGraph, Graph& orderedCs, vector<v8c>& rcs, vector<vertex>& xrcs, Graph& scs, int *clique_num, int rds) {
    vector<com_node> ancestors; // store all the vertices before the rth vertex of a r-clique
    vector<int> id_in_level(rds-1); // record total number of n-cliques where n = 3, 4, 5, 6, 7, 8
    int rounds = 3;
    for(size_t i1 = 0;i1 < (*root).nodes.size();i1++) {
        com_node *u1 = &root->nodes[i1];
        ancestors.push_back(*u1);
    ...
    *clique_num = id_in_level[rds-3];
    orderedCs.resize(*clique_num);
    scs.resize(*clique_num);
    // create r-clique list
    create_rcliqueList (root, orderedGraph, orderedCs, rcs, xrcs, scs, id_in_level,rds);
    return count9cliques_gen (root, graph, orderedGraph, orderedCs, rcs, scs);
}

void base_k89_gen (Graph& graph, bool hierarchy, edge nEdge, vector<vertex>& K, vertex* max45, string vfile, FILE* fp) {
    ...
    lol fc = create_cliques_and_count(&root, graph, orderedGraph, orderedCs, rcs, xrcs, scs, &clique_num, ROUNDS);

    fprintf (fp, "# 9-cliques: %lld\n", fc);
    const auto f2 = chrono::steady_clock::now();
    print_time (fp, "9-clique counting: ", f2 - f1);
