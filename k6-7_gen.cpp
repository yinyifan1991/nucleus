...
void traverse_cliqueTree (com_node* parent, vector<com_node>& ancestors, Graph& orderedGraph, Graph& orderedCs, vector<v6c>& rcs, vector<vertex>& xrcs, Graph& scs, vector<int>& id_in_level, int rounds) {
    if(rounds < 0) return;
    for(int i = 0;i < parent->nodes.size();i++) {
        ...
        if(rounds > 0) {
            ...
            for (auto x : commonNeighbors) {
                ...
                v6c cl = make_tuple (ancestors[0].val, ancestors[1].val, ancestors[2].val, ancestors[3].val, ancestors[4].val, (vertex)x);
                rcs.push_back (cl);
            }
...
void create_rcliqueList (com_node *root, Graph& orderedGraph, Graph& orderedCs, vector<v6c>& rcs, vector<vertex>& xrcs, Graph& scs, vector<int>& id_in_level) {
...
void create_cliqueList(com_node *root, com_node* parent, vector<com_node>& ancestors, Graph& orderedGraph, Graph& orderedCs, vector<v6c>& rcs, vector<vertex>& xrcs, Graph& scs, vector<int>& id_in_level, int rounds) {
    if(rounds > ROUNDS) {
        return;
    }
    vector<vector<vertex>> vList;
...
// per 7-clique
lol count8cliques_gen (com_node *root, Graph& graph, Graph& orderedGraph, Graph& orderedCs, vector<v6c>& rcs, Graph& scs) {
    lol fc = 0;
    for (auto t : rcs) {
        ...
        while (i1 < orderedGraph[u1].size() && i2 < orderedGraph[u2].size() && i3 < orderedGraph[u3].size()
            ...
            if (a1 == a2 && a1 == a3 && a1 == a4 && a1 == a5 && a1 == a6) {
                ...
                increment_clique(root, {u1, u2, u3, u4, u5, u6, x}, orderedCs, scs, clique, 0, 0, 1);
                cout << u1 << " " << u2 << " " << u3 << " " << u4 << " " << u5 << " " << u6 << " : " << x << endl;
                i1++; i2++; i3++; i4++, i5++, i6++;
                fc++;
            }
            else {
...
lol create_cliques_and_count(com_node *root, Graph& graph, Graph& orderedGraph, Graph& orderedCs, vector<v6c>& rcs, vector<vertex>& xrcs, Graph& scs, int *clique_num) {
...
void base_k67_gen (Graph& graph, bool hierarchy, edge nEdge, vector<vertex>& K, vertex* max45, string vfile, FILE* fp) {
    ...
    while (true) {
        ...
        cout << "get tuple " << u1 << " " << u2 << " " << u3 << " " << u4 << " " << u5 << " " << u6 << endl;
        vector<vector<vertex>> vList;
