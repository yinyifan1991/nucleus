...
inline void create_7cliqueList (com_node *root, Graph& orderedGraph, int *clique_num) {
    vertex counter = 0;
    for(size_t i1 = 0;i1 < (*root).nodes.size();i1++) {
...
inline void create_8cliqueList (com_node *root, Graph& orderedGraph, Graph& ordered8Cs, vector<v8c>& r8cs, vector<vertex>& xr8cs, Graph& s9cs) {
    vertex counter = 0;
    xr8cs.push_back(0);
    for(size_t i1 = 0;i1 < (*root).nodes.size();i1++) {
        com_node *u1 = &(*root).nodes[i1];
...
// per 4-clique
lol count9cliques (com_node *root, Graph& graph, Graph& orderedGraph, Graph& ordered8Cs, vector<v8c>& r8cs, Graph& s9cs) {
    ...
    for (auto t : r8cs) {
        ...
        while (i1 < orderedGraph[u1].size() && i2 < orderedGraph[u2].size() && i3 < orderedGraph[u3].size()
            ...
            if (a1 == a2 && a1 == a3 && a1 == a4 && a1 == a5 && a1 == a6 && a1 == a7 && a1 == a8) {
                vertex x = a1;
                vector<vertex> clique(8);
                increment_clique(root, {u1, u2, u3, u4, u5, u6, u7, u8, x}, ordered8Cs, s9cs, clique, 0, 0, 1);
...
void base_k89 (Graph& graph, bool hierarchy, edge nEdge, vector<vertex>& K, vertex* max45, string vfile, FILE* fp) {
    ...
    int clique_num = 0;
    create_7cliqueList (&root, orderedGraph, &clique_num);
    cout << "created 7-cliques" << endl;

    // Creating 8-clique list
    Graph ordered8Cs (clique_num); // like orderedGraph: each vector<vertex> is the list of rth vertices, ur, in the cliques of i-th r-1 cliques, u1 - u2 - u3... s.t. u1 < v2... < ur (deg ordering)
    vector<v8c> r8cs; // like el: list of the r-cliques aligned to the order in orderedCs
    vector<vertex> xr8cs; // like xel: indices in rcs that starts the r-clique list for an (r-1)-clique
    Graph s9cs (clique_num); // s-clique counts of each r-clique in the orderedCs structure
    cout << "start creating 8-cliques" << endl;
    create_8cliqueList (&root, orderedGraph, ordered8Cs, r8cs, xr8cs, s9cs);
    ...
    lol fc = count9cliques (&root, graph, orderedGraph, ordered8Cs, r8cs, s9cs);
    cout << "fc: " << fc << endl;
    ...
    K.resize (r8cs.size(), -1);
    Naive_Bucket nBucket;
    nBucket.Initialize (clique_num, r8cs.size()); // maximum 4-clique count of a triangle is nVtx

    vertex id = 0;
    for (size_t i = 0; i < s9cs.size(); i++)
        for (size_t j = 0; j < s9cs[i].size(); j++) {
            if (s9cs[i][j] > 0) {
                nBucket.Insert(xr8cs[i] + j, s9cs[i][j]);
                cout << i << " " << j << " " << xr8cs[i] + j<< " " << s9cs[i][j] << endl;
            }
            else
                K[xr8cs[i] + j] = 0;
            id++;
    ...
    while (true) {
        ...
        vertex u1 = get<0> (r8cs[t]);
        vertex u2 = get<1> (r8cs[t]);
        vertex u3 = get<2> (r8cs[t]);
        vertex u4 = get<3> (r8cs[t]);
        vertex u5 = get<4> (r8cs[t]);
        vertex u6 = get<5> (r8cs[t]);
        vertex u7 = get<6> (r8cs[t]);
        vertex u8 = get<7> (r8cs[t]);
        vector<vector<vertex>> vList;
        ...
        for (auto x : commonNeighbors) { // decrease the FC of the neighbor triangles with greater FC
            vertex v1 = getId(&root, {u1, u2, u3, u4, u5, u6, u7, x});
            vertex v2 = getId(&root, {u1, u2, u3, u4, u5, u6, u8, x});
            vertex v3 = getId(&root, {u1, u2, u3, u4, u5, u7, u8, x});
            vertex v4 = getId(&root, {u1, u2, u3, u4, u6, u7, u8, x});
            vertex v5 = getId(&root, {u1, u2, u3, u5, u6, u7, u8, x});
            vertex v6 = getId(&root, {u1, u2, u4, u5, u6, u7, u8, x});
            vertex v7 = getId(&root, {u1, u3, u4, u5, u6, u7, u8, x});
            vertex v8 = getId(&root, {u2, u3, u4, u5, u6, u7, u8, x});
