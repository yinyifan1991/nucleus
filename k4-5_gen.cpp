...
// build orderedCs, rcs and xrcs, preprocess scs
void traverse_cliqueTree (com_node* parent, vector<com_node>& ancestors, Graph& orderedGraph, Graph& orderedCs, vector<v4c>& rcs, vector<vertex>& xrcs, Graph& scs, vector<int>& id_in_level, int rounds) {
    if(rounds < 0) return;
    for(int i = 0;i < parent->nodes.size();i++) {
        com_node *u = &parent->nodes[i];
        ancestors.push_back(*u);
        // traverse tree until reach the last level
        if(rounds > 0) {
            ...
            for (auto x : commonNeighbors) {
                ...
                v4c cl = make_tuple (ancestors[0].val, ancestors[1].val, ancestors[2].val, (vertex)x);
                rcs.push_back (cl);
            }
...
// count number of s-clique per r-clique
lol count_cliques_gen (com_node *root, Graph& graph, Graph& orderedGraph, Graph& orderedCs, vector<v4c>& rcs, Graph& scs) {
    lol fc = 0;
    for (auto t : rcs) {
        vertex i1 = 0, i2 = 0, i3 = 0, i4 = 0;
        vertex u1 = get<0>(t);
        vertex u2 = get<1>(t);
        vertex u3 = get<2>(t);
        vertex u4 = get<3>(t);
        while (i1 < orderedGraph[u1].size() && i2 < orderedGraph[u2].size() && i3 < orderedGraph[u3].size()
               && i4 < orderedGraph[u4].size()) {
            ...
            if (a1 == a2 && a1 == a3 && a1 == a4) {
                ...
                increment_clique(root, {u1, u2, u3, u4, x}, orderedCs, scs, clique, 0, 0, 1);
                cout << u1 << " " << u2 << " " << u3 << " " << u4 << " : " << x << endl;
                i1++; i2++; i3++; i4++;
                fc++;
            }
            else {
                vertex mn = max ({a1, a2, a3, a4});
                if (a1 != mn)
                    i1++;
                if (a2 != mn)
                    i2++;
...
void create_rcliqueList (com_node *root, Graph& orderedGraph, Graph& orderedCs, vector<v4c>& rcs, vector<vertex>& xrcs, Graph& scs, vector<int>& id_in_level) {
...
// create clique list of orders from 3 to r-1
void create_cliqueList(com_node *root, com_node* parent, vector<com_node>& ancestors, Graph& orderedGraph, Graph& orderedCs, vector<v4c>& rcs, vector<vertex>& xrcs, Graph& scs, vector<int>& id_in_level, int rounds) {
    if(rounds > ROUNDS) {
        return;
    }
    vector<vector<vertex>> vList;
...
lol create_cliques_and_count(com_node *root, Graph& graph, Graph& orderedGraph, Graph& orderedCs, vector<v4c>& rcs, vector<vertex>& xrcs, Graph& scs, int *clique_num) {
...
void base_k45_gen (Graph& graph, bool hierarchy, edge nEdge, vector<vertex>& K, vertex* max45, string vfile, FILE* fp) {
    ...
    vector<v4c> rcs; // like el: list of the r-cliques aligned to the order in orderedCs
    ...
    while (true) {
        ...
        for (auto x : commonNeighbors) { // decrease the FC of the neighbor triangles with greater FC
            vertex v1 = getId(&root, {u1, u2, u3, x});
            vertex v2 = getId(&root, {u1, u2, u4, x});
            vertex v3 = getId(&root, {u1, u3, u4, x});
            vertex v4 = getId(&root, {u2, u3, u4, x});
            if ((v1 == -1 || K[v1] == -1) && (v2 == -1 || K[v2] == -1) &&
                ...
                createSkeleton (t, {v1, v2, v3, v4}, &nSubcores, K, skeleton, component, unassigned, relations);
        }
        if (hierarchy)
    ...
    *max45 = fc_t; // fc_t is fc of the last popped triangle
    ...
    if (!hierarchy) {
        ...
        print_time (fp, "Total 4,5 nucleus decomposition time (excluding density computation): ", (p2 - p1) + (f2 - f1) + (t2 - t1) + (b2 - b1));
        ...
        print_time (fp, "Total 4,5 nucleus decomposition time: ", (p2 - p1) + (f2 - f1) + (t2 - t1) + (b2 - b1) + (d2 - d1));
    }
}
