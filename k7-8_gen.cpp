...
void traverse_cliqueTree (com_node* parent, vector<com_node>& ancestors, Graph& orderedGraph, Graph& orderedCs, vector<v7c>& rcs, vector<vertex>& xrcs, Graph& scs, vector<int>& id_in_level, int rounds) {
    if(rounds < 0) return;
    for(int i = 0;i < parent->nodes.size();i++) {
        ...
        if(rounds > 0) {
            ...
            for (auto x : commonNeighbors) {
                ...
                v7c cl = make_tuple (ancestors[0].val, ancestors[1].val, ancestors[2].val, ancestors[3].val, ancestors[4].val, ancestors[5].val, (vertex)x);
                rcs.push_back (cl);
            }
...
void create_rcliqueList (com_node *root, Graph& orderedGraph, Graph& orderedCs, vector<v7c>& rcs, vector<vertex>& xrcs, Graph& scs, vector<int>& id_in_level) {
...
void create_cliqueList(com_node *root, com_node* parent, vector<com_node>& ancestors, Graph& orderedGraph, Graph& orderedCs, vector<v7c>& rcs, vector<vertex>& xrcs, Graph& scs, vector<int>& id_in_level, int rounds) {
    if(rounds > ROUNDS) {
        return;
    }
    vector<vector<vertex>> vList;
...
// per 7-clique
lol count8cliques_gen (com_node *root, Graph& graph, Graph& orderedGraph, Graph& orderedCs, vector<v7c>& rcs, Graph& scs) {
    lol fc = 0;
    for (auto t : rcs) {
        ...
        while (i1 < orderedGraph[u1].size() && i2 < orderedGraph[u2].size() && i3 < orderedGraph[u3].size()
            ...
            if (a1 == a2 && a1 == a3 && a1 == a4 && a1 == a5 && a1 == a6 && a1 == a7) {
                ...
                increment_clique(root, {u1, u2, u3, u4, u5, u6, u7, x}, orderedCs, scs, clique, 0, 0, 1);
...
lol create_cliques_and_count(com_node *root, Graph& graph, Graph& orderedGraph, Graph& orderedCs, vector<v7c>& rcs, vector<vertex>& xrcs, Graph& scs, int *clique_num) {
    ...
    *clique_num = id_in_level[ROUNDS-3];
    ...
    return count8cliques_gen (root, graph, orderedGraph, orderedCs, rcs, scs);
}
...
void base_k78_gen (Graph& graph, bool hierarchy, edge nEdge, vector<vertex>& K, vertex* max45, string vfile, FILE* fp) {
    ...
    cout << "fc: " << fc << endl;
    fprintf (fp, "# 8-cliques: %lld\n", fc);
    const auto f2 = chrono::steady_clock::now();
    print_time (fp, "8-clique counting: ", f2 - f1);
    ...
    for (size_t i = 0; i < scs.size(); i++) {
        for (size_t j = 0; j < scs[i].size(); j++) {
            if (scs[i][j] > 0) {
                ...
                //cout << i << " " << j << " " << xrcs[i] + j<< " " << scs[i][j] << endl;
            }
            else
                K[xrcs[i] + j] = 0;
            id++;
    ...
    while (true) {
        ...
        cout << "get tuple " << u1 << " " << u2 << " " << u3 << " " << u4 << " " << u5 << " " << u6 << " " << u7 << endl;
        vector<vector<vertex>> vList;
