...
unordered_map<v6c, int> clique6r_map; // store ids for (r-2)-clique
unordered_map<v7c, int> clique7s_map; // store ids for (r-1)-clique

void incrementCN(v6c cl, vertex u, Graph& orderedCs, Graph& scs) {
    if(clique6r_map.find(cl) != clique6r_map.end()) {
        int rid = clique6r_map[cl];
...
void increment_8clique(initializer_list<vertex> elements, Graph& orderedCs, Graph& scs) {
                ...
                for(size_t i4 = i3+1;i4 <= 4;i4++) {
                    ...
                    for(size_t i5 = i4+1;i5 <= 5;i5++) {
                        ...
                        for(size_t i6 = i5+1;i6 <= 6;i6++) {
                            ...
                            v6c cl = make_tuple(u1, u2, u3, u4, u5, u6);
                            for(size_t i7 = i6+1;i7 <= 7;i7++) {
                                vertex u7 = elements.begin()[i7];
                                incrementCN(cl, u7, orderedCs, scs);
                            }
                        }
                    }
                }
...
vertex getId(v7c cl) {
    if(clique7s_map.find(cl) != clique7s_map.end()) return clique7s_map[cl];
    return -1;
}
...
// per 7-clique
lol count_cliques (Graph& graph, Graph& orderedGraph, Graph& orderedCs, vector<v7c>& rcs, Graph& scs) {
    lol fc = 0;
    for (auto t : rcs) {
        ...
        while (i1 < orderedGraph[u1].size() && i2 < orderedGraph[u2].size() && i3 < orderedGraph[u3].size()
            ...
            if (a1 == a2 && a1 == a3 && a1 == a4 && a1 == a5 && a1 == a6 && a1 == a7) {
                vertex x = a1;
                increment_8clique({u1, u2, u3, u4, u5, u6, u7, x}, orderedCs, scs);
...
void base_k78_file (Graph& graph, bool hierarchy, edge nEdge, vector<vertex>& K, vertex* max45, string vfile, FILE* fp, string file_name) {
    ...
    int clique_num = 0;
    create_6cliqueList (orderedGraph, &clique_num, file_name, clique6r_map);
    ...
    vector<v7c> rcs; // like el: list of the r-cliques aligned to the order in orderedCs
    ...
    create_7cliqueList (orderedGraph, orderedCs, rcs, xrcs, scs, file_name, clique7s_map);
    ...
    while (true) {
        ...
        for (auto x : commonNeighbors) { // decrease the FC of the neighbor triangles with greater FC
            vertex v1 = getId(make_tuple(u1, u2, u3, u4, u5, u6, x));
            vertex v2 = getId(make_tuple(u1, u2, u3, u4, u5, u7, x));
            vertex v3 = getId(make_tuple(u1, u2, u3, u4, u6, u7, x));
            vertex v4 = getId(make_tuple(u1, u2, u3, u5, u6, u7, x));
            vertex v5 = getId(make_tuple(u1, u2, u4, u5, u6, u7, x));
            vertex v6 = getId(make_tuple(u1, u3, u4, u5, u6, u7, x));
            vertex v7 = getId(make_tuple(u2, u3, u4, u5, u6, u7, x));
    ...
    *max45 = fc_t; // fc_t is fc of the last popped triangle
    ...
    if (!hierarchy) {
        ...
        print_time (fp, "Total 7,8 nucleus decomposition time (excluding density computation): ", (p2 - p1) + (f2 - f1) + (t2 - t1) + (b2 - b1));
        ...
        print_time (fp, "Total 7,8 nucleus decomposition time: ", (p2 - p1) + (f2 - f1) + (t2 - t1) + (b2 - b1) + (d2 - d1));
    }
}
