...
unordered_map<vt, int> clique3r_map; // store ids for (r-2)-clique
unordered_map<v4c, int> clique4s_map; // store ids for (r-1)-clique

void incrementCN(vt cl, vertex u, Graph& orderedCs, Graph& scs) {
    if(clique3r_map.find(cl) != clique3r_map.end()) {
        int rid = clique3r_map[cl];
...
void increment_5clique(initializer_list<vertex> elements, Graph& orderedCs, Graph& scs) {
    for(size_t i1 = 0;i1 <= 1;i1++) {
        ...
        for(size_t i2 = i1+1;i2 <= 2;i2++) {
            ...
            for(size_t i3 = i2+1;i3 <= 3;i3++) {
                ...
                vt cl = make_tuple(u1, u2, u3);
                for(size_t i4 = i3+1;i4 <= 4;i4++) {
                    vertex u4 = elements.begin()[i4];
                    incrementCN(cl, u4, orderedCs, scs);
                }
            }
        }
    }
...
vertex getId(v4c cl) {
    if(clique4s_map.find(cl) != clique4s_map.end()) return clique4s_map[cl];
    return -1;
}
...
// per 7-clique
lol count_cliques (Graph& graph, Graph& orderedGraph, Graph& orderedCs, vector<v4c>& rcs, Graph& scs) {
    lol fc = 0;
    for (auto t : rcs) {
        ...
        while (i1 < orderedGraph[u1].size() && i2 < orderedGraph[u2].size() && i3 < orderedGraph[u3].size()
            ...
            if (a1 == a2 && a1 == a3 && a1 == a4) {
                vertex x = a1;
                increment_5clique({u1, u2, u3, u4, x}, orderedCs, scs);
                i1++; i2++; i3++; i4++;
                fc++;
            }
            else {
...
void base_k45_file (Graph& graph, bool hierarchy, edge nEdge, vector<vertex>& K, vertex* max45, string vfile, FILE* fp, string file_name) {
    ...
    int clique_num = 0;
    create_triangleList (orderedGraph, &clique_num, file_name, clique3r_map);
    ...
    create_4cliqueList (orderedGraph, orderedCs, rcs, xrcs, scs, file_name, clique4s_map);
    ...
    nBucket.Initialize (clique_num, rcs.size());
    ...
    while (true) {
        ...
        for (auto x : commonNeighbors) { // decrease the FC of the neighbor triangles with greater FC
            vertex v1 = getId(make_tuple(u1, u2, u3, x));
            vertex v2 = getId(make_tuple(u1, u2, u4, x));
            vertex v3 = getId(make_tuple(u1, u3, u4, x));
            vertex v4 = getId(make_tuple(u2, u3, u4, x));
